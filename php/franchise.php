...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    extract($_POST);
    $sql = "INSERT INTO franchise(name,email,phone) VALUES('$name', '$email', '$phone')";
    require 'db.inc.php';
    ...
    if ($conn->query($sql)) {
        ...
        $from = "$name <$email>";
        $subject = "Franchise Request from Anaforwomen Website";
        $body = '<!DOCTYPE html>
        <html>
        ...
        <body style="font-family: Arial, Helvetica, sans-serif;margin:0px;background-color: #ffffff;">
            <table style="background-color: #eeeeee;padding: 8px 16px;width: 100%;box-shadow: 0 2px 2px 0 rgba(0,0,0,0.14), 0 3px 1px -2px rgba(0,0,0,0.12), 0 1px 5px 0 rgba(0,0,0,0.2);">
                <tr>
                    <td><img src="https://www.anaforwomen.com/images/logo.png" height="50px" alt="Anaforwoman" /></td>
                    <td style="line-height: 50px;vertical-align: top; margin:0px; font-size: 32px; font-weight: 500;">Franchise Request from Anaforwomen Website</td>
                </tr>
            </table>
            <table style="padding: 8px 16px;width: 100%;font-weight: 500;" cellspacing="10">
                ...
                <tr>
                    <td style="color: #263a73;width: 30%">Phone:</td>
